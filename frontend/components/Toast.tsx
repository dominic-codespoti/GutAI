...
const DARK_COLORS: Record<
  ...
  { bg: string; border: string; text: string; icon: string }
> = {
  error: { bg: "#1f0a0a", border: "#991b1b", text: "#fca5a5", icon: "✕" },
...
function ToastItem({
  ...
  const c = isDark ? DARK_COLORS[type] : LIGHT_COLORS[type];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text
          style={{ fontSize: 18, color: c.text, paddingLeft: 8, opacity: 0.5 }}
        >
          ×
        </Text>
      </TouchableOpacity>
