...
function parsedItemWarnings(
  ...
  if (item.resolutionStatus === "Unresolved") {
    ...
    warnings.push({
      text: "Estimated match — verify this item",
      tone: "warning",
    });
  }
  ...
  if (item.nutritionProvenance === "Unknown") {
    warnings.push({
      text: "Nutrition unknown — edit values manually",
      tone: "danger",
    });
  ...
  if (item.portionConfidence < 0.5) {
    warnings.push({
      text: "Portion assumed — default serving size",
      tone: "muted",
    });
  }
...
export function LogMealSheet() {
  ...
  const scrollMaxHeight = Math.max(180, Math.min(420, windowHeight * 0.5));
  ...
  const [mealMinute, setMealMinute] = useState(
  ...
  const aiDescribeMutation = useMutation({
    ...
    onError: () => {
      toast.error(
        "Could not generate food details from that description. Try being more specific.",
      );
    },
  });
        ...
        items: [
          {
            ...
            foodProductId:
              product.id !== EMPTY_PRODUCT_ID ? product.id : undefined,
            servings: 1,
            servingUnit: `${s}g`,
            servingWeightG: s,
            ...scaled,
      ...
      >
        {subView === "swap" ? (
          ...
          <View style={sheetContentRootStyle}>
              ...
              {tabs.map(({ key, label, icon }) => (
                ...
                >
                  <Ionicons
                    ...
                    size={14}
                    color={
                      activeTab === key ? colors.primary : colors.textMuted
                    }
                  />
                  <Text
                    style={{
              ...
              <View>
                ...
                >
                  ...
                  <TouchableOpacity
                    ...
                    disabled={
                      aiDescribeMutation.isPending || !naturalText.trim()
                    }
                    style={{
              ...
              <View>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Text style={{ fontSize: 12, color: colors.textMuted }}>
                        Serving: {aiPreview.servingSize}
                        {aiPreview.servingSizeUnit}
                      </Text>
                    </View>
                    <View
                      style={{
                  ...
                  {aiPreview.extractionConfidence != null && (
                    ...
                    >
                      ...
                      />
                      <Text style={{ fontSize: 12, color: colors.textMuted }}>
                        {aiPreview.extractionConfidence >= 0.85
                          ? "High"
                          : aiPreview.extractionConfidence >= 0.6
                            ? "Medium"
                            : "Low"}{" "}
                        confidence (
                        {Math.round(aiPreview.extractionConfidence * 100)}%)
                      </Text>
                    </View>
                  )}
                  ...
                  >
                    This food will be saved to My Foods and logged to{" "}
                    {selectedDate} · {selectedMealType}.
                  </Text>
                </View>
                ...
                >
                  ...
                  <TouchableOpacity
                    onPress={handleAiLog}
                    disabled={
                      createMeal.isPending || aiDescribeMutation.isPending
                    }
                    style={{
                      backgroundColor: colors.primary,
                      paddingHorizontal: 16,
                      ...
                      {searchResults.data && searchResults.data.length > 0 && (
                        <View style={{ marginTop: 8 }}>
                          ...
                          {searchResults.data.map(
                            (product: FoodProduct, idx: number) => (
                              <FoodSearchResult
                      ...
                      {[
                        {
                          key: "recent",
                          title: "Recent",
                          data: (recentFoodsQuery.data ?? []).map(
                            (f: RecentFood, i: number) => ({
                              key: `r-${f.foodProductId ?? f.foodName}-${i}`,
                              product: mapRecentFoodToProduct(f),
                            }),
                          ),
                        },
                        {
                          key: "favorites",
                          title: "Favorites",
                          data: favorites.map((f: FavoriteFood, i: number) => ({
                            key: `fav-${f.foodProductId}-${i}`,
                            product: mapFavoriteFoodToProduct(f),
                          })),
                        },
                        {
                          key: "my-foods",
                          title: "My Foods",
                          data: (customFoodsQuery.data ?? []).map(
                            (f: FoodProduct, i: number) => ({
                              key: `mf-${f.id ?? i}`,
                              product: f,
                            }),
                          ),
                              ...
                              {section.data.map(
                                (item: {
                                  ...
                                  <FoodSearchResult
                                    ...
                                    onDetailPress={
                                      item.product.id &&
                                      item.product.id !== EMPTY_PRODUCT_ID
                                        ? () =>
                                            router.push(
            ...
            {activeTab === "scan" && (
              <ScrollView
                style={[sheetScrollStyle, { maxHeight: 440 }]}
                showsVerticalScrollIndicator={false}
                keyboardShouldPersistTaps="handled"
              >
                ...
                >
                  ...
                  >
                    Photograph your plate — AI detects components, portions &
                ...
                >
                  ...
                  >
                    Scan UPC / EAN barcode on packaged grocery products for
                    instant lookup.
                  </Text>
                ...
                >
                  ...
                  >
                    Photograph the back-of-box nutrition panel to extract facts
                    into custom food.
                  </Text>
